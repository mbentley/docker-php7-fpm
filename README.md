...
docker image for php7.0-fpm
based off of debian:stretch

To pull this image:
`docker pull mbentley/php7.0-fpm`

Example usage:
`docker run -i -t mbentley/php7.0-fpm`
...
`docker run -itd –restart=always -v /data/shared/run:/run -v /data/www:/data/www -v /data/shared/ssmtp:/etc/ssmtp –name php7.0-fpm mbentley/php7.0-fpm`
...
I need to present /data/shared/run as a volume so that nginx can read the UNIX socket (see my [php.conf](https://github.com/mbentley/docker-nginx/blob/master/php.conf) in [mbentley/nginx](https://github.com/mbentley/docker-php7.0-fpm) which adds all of the necessary bits for PHP by including a single file). I also add in my /data/www directory which shares my site's code for nginx.
